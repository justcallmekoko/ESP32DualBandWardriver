...
uint8_t Utils::getDRAMUsagePercent() {
...
#ifdef HAS_PSRAM
  uint8_t Utils::getPSRAMUsagePercent() {
...
String Utils::byteArrayToHexString(const std::vector<uint8_t>& byteArray) {
  String result;
...
std::vector<uint8_t> Utils::hexStringToByteArray(const String& hexString) {
...
void Utils::generateRandomName(char *name, size_t length) {
...
const char* Utils::generateRandomName() {
...
void Utils::generateRandomMac(uint8_t* mac) {
...
String Utils::macToString(const Station& station) {
...
String Utils::macToString(uint8_t macAddr[6]) {
...
String Utils::replaceOUIWithManufacturer(const char *sta_addr) {
...
IPAddress Utils::getNextIP(IPAddress currentIP, IPAddress subnetMask) {
